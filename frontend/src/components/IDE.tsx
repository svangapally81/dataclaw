import { CheckCircle2, ChevronDown, Database, Send, Sparkles, StopCircle, XCircle } from "lucide-react";
  ...
  async function handleAsk(question: string) {
    ...
    try {
      ...
      if (response.sql) {
        ...
        setLiveRows((prev) => {
          const next = { ...prev };
          delete next[response.thread_id];
          return next;
        });
      }
    } catch (err) {
      ...
      <div className="editor-stream" ref={scrollRef}>
        ...
        {messages.map((message) => (
          <ChatBubble
            key={message.id}
            message={message}
            liveRows={message.sql && message.id === latestAssistantMessageId && activeThreadId ? liveRows[activeThreadId] ?? [] : []}
    ...
    <div className="bubble assistant">
      ...
      {isPendingApproval && message.alert_id ? (
        <div className="chat-approval">
          ...
          {effectiveToolResult ? <JsonExpander label="Advanced payload" value={effectiveToolResult} /> : null}
        </div>
      ) : null}
      {message.sql ? (
...
function JsonExpander({ label, value }: { label: string; value: unknown }) {
  let pretty = "";
  try {
  ...
  return (
    <details className="chat-approval-json">
