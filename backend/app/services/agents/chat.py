...
JOIN core.customers c ON c.id = o.customer_id
WHERE r.issued_at >= CURRENT_DATE - INTERVAL '90 days'
...
"""
        ...
        rows = result.get("rows") if isinstance(result.get("rows"), list) else []
        return _with_retrieval_trace({
            "answer": f"There are {sum(int(row.get('refund_count') or 0) for row in rows)} refund(s) in the last 90 days.",
...
async def answer_question(
    ...
    direct_scenario_answer = await _scenario6_direct_answer(
        ...
        user_email=user_email,
        provider_slug=provider_slug,
        retrieval_trace={key: value for key, value in retrieval_trace.items() if key != "warning"},
        run_id=run_id,
    )
