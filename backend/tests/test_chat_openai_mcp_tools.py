...
@pytest.mark.asyncio
async def test_scenario6_airflow_dag_creation_returns_created_message(mcp_session, monkeypatch) -> None:
    session, tool_engine, agent = mcp_session
    seen: dict[str, object] = {}

    async def fake_direct_mcp_call(**kwargs):
        seen.update(kwargs)
        return {"status": "created", "dag": {"dag_id": kwargs["arguments"]["dag_id"]}}
    ...
    assert seen["arguments"]["dag_id"] == "weekly_revenue"
    assert "__approved" not in seen["arguments"]
    assert payload["answer"] == "Created the Airflow DAG weekly_revenue. It materializes weekly_revenue every Monday at 06:00."
    assert payload["tool_call"] == {"connector_slug": "airflow", "tool": "write_create_dag"}
